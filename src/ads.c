 ...
 */
void
ads_debug (int type, const char *fmt, ...)
{
  va_list args;
  char tmp_str[1000] = { 0, };
  if (ADSDebug & type)
...
void
_ADSDumpAMSNetId (AMSNetID * id)
{
  _ADSDump ("AMSNetID: ", (uc *) id, sizeof (AMSNetID));
}

void
_ADSDumpAMSheader (AMSheader * h)
{
  if (!ADSDebug)
    return;
 ...
 */
int
freeADSInterface (ADSInterface * di)
{
  free (di);
  return 0;
}
 ...
 */
int
freeADSConnection (ADSConnection * dc)
{
...
*/
void
_ADSDump (char *name, void *v, int len)
{
  if (!ADSDebug)
    return;
  uc *b = (uc *) v;
...
#ifdef __linux__
int
_ADSReadOne (ADSInterface * di, uc * b)
{
  fd_set FDS;
  struct timeval t;
  FD_ZERO (&FDS);
...
*/
int
_ADSReadPacket (ADSInterface * di, uc * b)
{
...
void
_ADSsetupHeader (ADSConnection * dc, AMSheader * h)
{
...
int
_ADSwrite2 (ADSInterface * di, void *buffer, int len)
{
  int res;
  if (di == NULL)
    return -2;
...
int
ADSparseNetID (const char *netIDstring, AMSNetID * id)
{
  const char *p;
  char *q;
  int i;
 ...
 */
ADSConnection *
AdsSocketConnect (int *socket_fd, PAmsAddr pAddr, PAmsAddr pMeAddr)
{
